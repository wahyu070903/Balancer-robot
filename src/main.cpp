...
#define SAVETY_TILT_FRONT -15.0
#define SAVETY_TILT_BACK 32.0
#define DIGITAL_COUNTER_SAVETY_UP 1.10
#define DIGITAL_COUNTER_SAVETY_DOWN 0.0
...
uint8_t allowed_gamepadMac[6] = {0xA0, 0xB7, 0x65, 0x14, 0xB7, 0xAE};
...
float digitalCounter = 0.0;
float ThrottleFactorNormal = 0.32;
float ThrottleFactorBoost = 0.62;
float linearThrottleFactor = ThrottleFactorNormal;
...
const unsigned long GRIP_COMPENSATION_DELAY = 1000;
...
void processGamepad(ControllerPtr ctl) {
  ...
  if (ctl->buttons() & 0x0008) {
    ...
    if(digitalCounter <= DIGITAL_COUNTER_SAVETY_UP){
  ...
  if (ctl->buttons() & 0x0002) {
    ...
    if(digitalCounter >= DIGITAL_COUNTER_SAVETY_DOWN){
      if(isLifterUp == false){
        isLifterDown = true;
        digitalCounter -= 0.1;
      }
  ...
  if (ctl->buttons() & 0x0040) {
    // L2 pressed
    linearThrottleFactor = ThrottleFactorBoost;
  } else {
    // L2 released
    linearThrottleFactor = ThrottleFactorNormal;
  }
  ...
  if(abs(ly) > 25){
    if (xSemaphoreTake(pidDataMutex, portMAX_DELAY) == pdTRUE) {
      throttle_input = ExponentialResponse(ly * linearThrottleFactor, MAX_MOTOR_RPM);
...
void SerialPIDTune() {
  ...
  if (newCommand) {
    ...
    if(serialInput.startsWith("Orient")){
      ...
      serial_tuneOpt = 106;
    }else if(serialInput.startsWith("Linear")){
      serial_tuneOpt = 107;
    }
    ...
    else if(serial_tuneOpt == 107){
      linearThrottleFactor = serialInput.toFloat();
      Serial.print("Updated Linear Throttle Factor"); Serial.println(linearThrottleFactor);
    }
...
float ExponentialResponse(int input, float maxValue){
  ...
  float exponential = sign * pow(abs(normalized), 0.8); // 1.5
  ...
  for(;;){
    ...
    if(dmp_ready){
      ...
      if (xSemaphoreTake(pidDataMutex, portMAX_DELAY) == pdTRUE){
        ...
        if (input > SAVETY_TILT_FRONT && input < SAVETY_TILT_BACK){
          isRobotFall = false;
